...
;; waiting task's own readiness condition is never met (test 1).
;;
;; In contrast, a task waiting to *start* (parked at the entry gate behind the
;; lock) is always cancellable, since cancelling it doesn't run any of the
;; callee's core wasm at all (test 2).

;; Test 1: a task parked in its `callback` event loop is woken to receive a
;; pending cancellation request as soon as the exclusive lock frees.
(component
  (component $C
...
;; Test 2: a task parked at the entry gate waiting for the exclusive lock is
;; cancellable even while the lock is held, and cancelling it resolves
;; immediately without running any of the callee's core wasm.
(component
  (component $C
    (core module $Memory (memory (export "mem") 1))
    (core instance $memory (instantiate $Memory))
    (core module $CM
      ...
      ;; Async-typed but lifted with the sync ABI, so this task holds the
      ;; exclusive lock for the whole call, including while it is blocked.
      (func (export "hold-lock") (param $futr i32) (result i32)
        (local $ws i32)
        (if (i32.ne (i32.const -1 (; BLOCKED ;))
                    (call $future.read (local.get $futr) (i32.const 0)))
      ...
      ;; Needs the exclusive lock to start, so it parks at the entry gate and
      ;; must never run: cancelling it while it is parked aborts it before it
      ;; is ever started.
      (func (export "never-runs") (result i32)
        unreachable
      )
      (func (export "unreachable-cb") (param i32 i32 i32) (result i32)
        unreachable
    ...
    (func (export "never-runs") async (result u32) (canon lift
      (core func $cm "never-runs")
      async (callback (core func $cm "unreachable-cb"))
    ))
  )
  ...
  (component $D
    (type $FT (future))
    (import "hold-lock" (func $hold-lock async (param "fut" $FT) (result u32)))
    (import "never-runs" (func $never-runs async (result u32)))
    ...
    (core module $DM
      ...
      (import "" "waitable-set.wait" (func $waitable-set.wait (param i32 i32) (result i32)))
      (import "" "hold-lock" (func $hold-lock (param i32 i32) (result i32)))
      (import "" "never-runs" (func $never-runs (param i32) (result i32)))

      (func $run (export "run") (result i32)
        (local $ret i32) (local $ret64 i64)
        (local $sub-hold i32) (local $sub-never i32)
        (local $futr i32) (local $futw i32) (local $ws i32)

        ;; start the lock holder; it blocks mid-body still holding the lock
        (local.set $ret64 (call $future.new))
        (local.set $futr (i32.wrap_i64 (local.get $ret64)))
        (local.set $futw (i32.wrap_i64 (i64.shr_u (local.get $ret64) (i64.const 32))))
        (local.set $ret (call $hold-lock (local.get $futr) (i32.const 0)))
        (if (i32.ne (i32.const 1 (; STARTED ;)) (i32.and (local.get $ret) (i32.const 0xf)))
          (then unreachable))
        (local.set $sub-hold (i32.shr_u (local.get $ret) (i32.const 4)))

        ;; the second call can't acquire the lock, so it parks before starting
        (local.set $ret (call $never-runs (i32.const 4)))
        (if (i32.ne (i32.const 0 (; STARTING ;)) (i32.and (local.get $ret) (i32.const 0xf)))
          (then unreachable))
        (local.set $sub-never (i32.shr_u (local.get $ret) (i32.const 4)))

        ;; even though the lock is still held, a task that hasn't started is
        ;; cancellable, so this completes immediately instead of blocking
        (local.set $ret (call $subtask.cancel (local.get $sub-never)))
        (if (i32.ne (i32.const 3 (; CANCELLED_BEFORE_STARTED ;)) (local.get $ret))
          (then unreachable))
        (call $subtask.drop (local.get $sub-never))

        ;; let the lock holder finish
        (if (i32.ne (i32.const 0 (; COMPLETED ;))
                    (call $future.write (local.get $futw) (i32.const 0)))
          (then unreachable))
        (local.set $ws (call $waitable-set.new))
        ...
        (if (i32.ne (i32.const 2 (; RETURNED ;)) (i32.load offset=4 (i32.const 8)))
          (then unreachable))
        (if (i32.ne (i32.const 44) (i32.load (i32.const 0)))
          (then unreachable))
        (call $subtask.drop (local.get $sub-hold))
    ...
    (canon waitable-set.wait (memory (core memory $memory "mem")) (core func $waitable-set.wait))
    (canon lower (func $hold-lock) async (memory (core memory $memory "mem")) (core func $hold-lock'))
    (canon lower (func $never-runs) async (memory (core memory $memory "mem")) (core func $never-runs'))
    (core instance $dm (instantiate $DM (with "" (instance
      (export "mem" (memory $memory "mem"))
      (export "subtask.cancel" (func $subtask.cancel))
      (export "subtask.drop" (func $subtask.drop))
      ...
      (export "waitable-set.wait" (func $waitable-set.wait))
      (export "hold-lock" (func $hold-lock'))
      (export "never-runs" (func $never-runs'))
    ))))
    (func (export "run") async (result u32) (canon lift (core func $dm "run")))
  )
  ...
  (instance $d (instantiate $D
    (with "hold-lock" (func $c "hold-lock"))
    (with "never-runs" (func $c "never-runs"))
  ))
  (func (export "run") (alias export $d "run"))
)
(assert_return (invoke "run") (u32.const 42))
