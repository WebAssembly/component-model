...
(component definition $Tester
  ...
  (core module $M
    ...
    (func (export "test-no-read-tuple")
      (local $ret i32) (local $ret64 i64)
      (local $rx i32) (local $tx i32)
    ...
    (func (export "test-no-write-tuple")
      (local $ret i32) (local $ret64 i64)
      (local $rx i32) (local $tx i32)
      (local.set $ret64 (call $future.newc))
      (local.set $rx (i32.wrap_i64 (local.get $ret64)))
  ...
  (type $FT (future))
  (type $ST (stream))
  (type $FTupT (future (tuple char)))
  (type $STupT (stream (tuple char)))
  ...
  (canon future.new $FTupT (core func $future.newc))
  (canon future.read $FTupT async (memory $memory "mem") (core func $future.readc))
  (canon future.write $FTupT async (memory $memory "mem") (core func $future.writec))
  (canon stream.new $STupT (core func $stream.newc))
  (canon stream.read $STupT async (memory $memory "mem") (core func $stream.readc))
  (canon stream.write $STupT async (memory $memory "mem") (core func $stream.writec))
  ...
  (func (export "test-no-read-tuple") (canon lift (core func $m "test-no-read-tuple")))
  (func (export "test-no-write-tuple") (canon lift (core func $m "test-no-write-tuple")))
)
(component instance $i $Tester)
(assert_return (invoke "test-empty") (u32.const 42))
(component instance $i $Tester)
(assert_trap (invoke "test-no-read-tuple") "cannot read from and write to intra-component future")
(component instance $i $Tester)
(assert_trap (invoke "test-no-write-tuple") "cannot read from and write to intra-component future")
