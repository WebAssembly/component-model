...
(component definition $Join
  (component $C
    ...
    (core module $Core
      ...
      ;; built-ins reject a dropped handle. These run on a fresh instance that
      ;; has not called "setup", so `thread.get-task` hands out index 1.
      (func (export "dropped-set")
        (local $t i32)
      ...
      (func (export "zero-set")
        (call $thread.set-task (i32.const 0))
        unreachable)
      (func (export "zero-drop")
    ...
    (func (export "dropped-set") async (result u32)
      (canon lift (core func $core "dropped-set") async))
    (func (export "double-drop") async (result u32)
      (canon lift (core func $core "double-drop") async))
    (func (export "zero-set") async (result u32)
      (canon lift (core func $core "zero-set") async))
    (func (export "zero-drop") async (result u32)
      (canon lift (core func $core "zero-drop") async))
    (func (export "reuse") async (result u32)
      (canon lift (core func $core "reuse") async))
  )
...
(component
  (component $C
    (core module $Core
      ...
      ;; task B: stackful async, so it never touches the exclusive lock. Its
      ;; implicit thread parks forever; B is resolved by $holder's thread.
      (func (export "target")
        (global.set $target-task (call $thread.get-task))
        (drop (call $thread.suspend))
        unreachable)
      ...
      (func (export "holder") (result i32)
        ...
        ;; a sync lift has to return in its original task
        (call $thread.set-task (local.get $home))
        (call $task.drop (local.get $home))
        (call $task.drop (global.get $target-task))
        (i32.const 42))
  ...
  (component $D
    ...
    (core module $Core
      ...
      (func (export "run") (result i32)
        ...
        ;; start target and let its implicit thread park
...
(component
  (component $C
    (core module $Core
      ...
      ;; task B: park the implicit thread; B is never resolved and never
      ...
      (func (export "abandon") (result i32)
        (call $thread.set-task (global.get $target-task))
        (i32.const 0 (; EXIT ;)))
      (func (export "abandon-cb") (param i32 i32 i32) (result i32)
        unreachable)
    ...
    (func (export "abandon") async (result u32)
      (canon lift (core func $core "abandon")
        async (callback (core func $core "abandon-cb"))))
