  ...
  async def enter(self, thread):
    if self.opts.sync or self.opts.callback:
      if self.inst.lock.locked():
        ...
        if await self.block_on(thread, acquired, cancellable = True) == Cancelled.TRUE:
          if acquired.done():
    ...
    if not self.inst.no_backpressure.is_set() or self.inst.num_backpressure_waiters > 0:
      while True:
        ...
        cancelled = await self.block_on(thread, maybe_go, cancellable = True, unlock = False)
        self.inst.num_backpressure_waiters -= 1
        if cancelled:
          return Cancelled.TRUE
...
def canon_lift(opts, inst, ft, callee, caller, on_start, on_resolve):
  ...
  async def async_impl():
    ...
    if await task.enter(thread) == Cancelled.TRUE:
      task.cancel()
      task.exit()
      return
