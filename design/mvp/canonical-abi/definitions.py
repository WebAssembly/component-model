...
class StreamOrFuture:
  t: ValType
  readable_end: StreamOrFutureEnd
  writable_end: StreamOrFutureEnd
  dropped: bool
  ...
  def forward_to(self, other):
    ...
    if other.dropped:
      ...
      self.read(other.pending_inst, other.pending_buffer, other.pending_on_copy_done,
                other.pending_on_partial_copy)
      other.reset_pending()

class Stream(StreamOrFuture):
  def __init__(self, t):
    StreamOrFuture.__init__(self, t, ReadableStreamEnd(self), WritableStreamEnd(self))
...
class StreamOrFutureEnd(Waitable):
  state: EndState
  shared: Stream|Future

  def __init__(self, shared):
    Waitable.__init__(self)
    self.state = EndState.IDLE
    self.shared = shared

  def copying(self):
    match self.state:
      case EndState.IDLE | EndState.DONE:
        return False
      case EndState.COPYING | EndState.CANCELLING_COPY:
        return True
    assert(False)
...
class ReadableStreamEnd(StreamOrFutureEnd):
  pass

class WritableStreamEnd(StreamOrFutureEnd):
  pass

### Future State

class Future(StreamOrFuture):
  def __init__(self, t):
    StreamOrFuture.__init__(self, t, ReadableFutureEnd(self), WritableFutureEnd(self))
...
class ReadableFutureEnd(StreamOrFutureEnd):
  pass

class WritableFutureEnd(StreamOrFutureEnd):
  def drop(self):
    trap_if(self.state != EndState.DONE)
    StreamOrFutureEnd.drop(self)
...
def lift_async_value(ReadableEndT, cx, i, t):
  ...
  trap_if(end.state != EndState.IDLE)
...
def lower_async_value(ReadableEndT, cx, end, t):
  ...
  assert(end.state == EndState.IDLE)
...
def stream_copy(EndT, BufferT, copy, event_code, stream_t, opts, i, ptr, n):
  ...
  def deliver_stream_event(result: Optional[CopyResult]):
    ...
    if result == CopyResult.DROPPED:
      end.state = EndState.DONE
    else:
      end.state = EndState.IDLE
    assert(0 <= result < 2**4)
...
def cancel_copy(EndT, event_code, stream_or_future_t, async_, i):
  ...
  trap_if(end.state != EndState.COPYING or end.has_sync_waiter)
  trap_if(end.in_waitable_set() and not async_)
  end.state = EndState.CANCELLING_COPY
  if not end.has_pending_event():
    end.shared.cancel()
    if not end.has_pending_event():
      if not async_:
...
def forward_copy(ReadableEndT, WritableEndT, stream_or_future_t, ri, wi):
  ...
  trap_if(w.state != EndState.IDLE)
  trap_if(w.in_waitable_set())
  if r.shared is not w.shared:
    r.shared.forward_to(w.shared)
  return []
