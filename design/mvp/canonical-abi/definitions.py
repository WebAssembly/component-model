...
class ComponentInstance:
  ...
  def flip_may_enter_to(self, may_enter):
    inst = self
    while inst:
      assert(inst.may_enter != may_enter)
      inst.may_enter = may_enter
      inst = inst.parent
...
def call_might_be_recursive(c1, c2):
  return c1 is c2 or c1.is_ancestor_of(c2) or c2.is_ancestor_of(c1)
...
class Thread:
  ...
  def resume(self, cancelled = Cancelled.FALSE):
    ...
    if self.task:
      if current_thread() and current_thread().task:
        trap_if(call_might_be_recursive(current_thread().task.inst, self.task.inst))
      self.task.inst.flip_may_enter_to(False)
    if self.waiting():
    ...
    if self.task:
      self.task.inst.flip_may_enter_to(True)
...
class Store:
  ...
  def invoke(self, f: FuncInst, on_start: OnStart, on_resolve: OnResolve) -> OnCancel:
    assert(not current_thread() or current_thread().store is self)
    return f(on_start, on_resolve)
...
class Task:
  ...
  def request_cancellation(self):
    if self.state == Task.State.INITIAL:
      ...
      assert(self.state == Task.State.ENTERED)
      candidates = { t for t in self.threads if t.cancellable }
      if self.needs_exclusive() and self.inst.exclusive and self.inst.exclusive is not self:
  ...
  def suspend_to_suspended(self, cancellable, other: Thread) -> Cancelled:
    if self.deliver_pending_cancel(cancellable):
      return Cancelled.TRUE
    return current_thread().suspend_to_suspended(cancellable, other)

  def yield_to_suspended(self, cancellable, other: Thread) -> Cancelled:
    if self.deliver_pending_cancel(cancellable):
      return Cancelled.TRUE
    return current_thread().yield_to_suspended(cancellable, other)

  def suspend_then_promote(self, cancellable, other: Thread) -> Cancelled:
    if self.deliver_pending_cancel(cancellable):
      return Cancelled.TRUE
    return current_thread().suspend_then_promote(cancellable, other)

  def yield_then_promote(self, cancellable, other: Thread) -> Cancelled:
    if self.deliver_pending_cancel(cancellable):
      return Cancelled.TRUE
    return current_thread().yield_then_promote(cancellable, other)
...
def canon_lift(opts, inst, ft, callee, on_start, on_resolve) -> OnCancel:
    ...
    while code != CallbackCode.EXIT:
      ...
      match code:
        case CallbackCode.YIELD:
          cancelled = thread.wait_until(lambda: not inst.exclusive, Cancellable.TRUE)
          if cancelled:
            event = (EventCode.TASK_CANCELLED, 0, 0)
          else:
  ...
  if not ft.async_:
    while task.state != Task.State.RESOLVED:
      candidates = { t for t in inst.threads if t.ready() and t is not t.task.implicit_thread }
...
def canon_thread_yield(cancellable):
  task = current_thread().task
  trap_if(not task.inst.may_leave)
  cancelled = task.wait_until(lambda: True, cancellable)
  return [cancelled]
...
def canon_thread_suspend_to_suspended(cancellable, i):
  ...
  trap_if(not other_thread.suspended())
  cancelled = task.suspend_to_suspended(cancellable, other_thread)
  return [cancelled]
...
def canon_thread_yield_to_suspended(cancellable, i):
  ...
  other_thread = task.inst.threads.get(i)
  trap_if(not other_thread.suspended())
  cancelled = task.yield_to_suspended(cancellable, other_thread)
  return [cancelled]
...
def canon_thread_suspend_then_promote(cancellable, i):
  ...
  trap_if(not task.inst.may_leave)
  other_thread = task.inst.threads.get(i)
  cancelled = task.suspend_then_promote(cancellable, other_thread)
  return [cancelled]
...
def canon_thread_yield_then_promote(cancellable, i):
  ...
  trap_if(not task.inst.may_leave)
  other_thread = task.inst.threads.get(i)
  cancelled = task.yield_then_promote(cancellable, other_thread)
  return [cancelled]
