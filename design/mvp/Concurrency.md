...
* Complement, but don't depend on, the Core WebAssembly proposals:
  * [stack-switching]
...
statically see that there is never a need to switch between active callstacks
and thus make a plain synchronous function call with the same wasm-observable
behavior. But in general, when one component makes an async call that
transitively blocks in another component, having the callee on its own native
callstack is needed for the runtime to be able to switch back to the caller
...
threads only at Component Model-defined points (like import calls). While these
**cooperative threads** do not allow a single component instance to increase
its internal parallelism, cooperative threads are still quite useful for
getting existing threaded code to Just Work (as-if running on a single core)
without the overhead of [CPS Transform] techniques like [Asyncify]. Moreover,
in various embeddings, available parallelism is already saturated by running
separate component instances on separate kernel threads.
...
the choice of linear vs. managed memory). This enables component authors to
...
creates a new ([green][green thread]) **thread** which, in many cases, can be
optimized into a synchronous function call.

Additionally, each call to a component export also creates a new **task** which
immutably *contains* the new thread. Whereas a *thread* contains a callstack
and other suspend/resume execution state, a *task* contains bookkeeping state
used to enforce the Canonical ABI rules associated with the export call. Tasks
are themselves contained by the component instance whose export was called.
Thus, the overall containment relationship is:
```
Component Store
  ↓ contains
Component Instance
...
thread by calling [`thread.new_indirect`], the new thread inherits the task of
the original thread. Thus there is an N:1 relationship between threads and
tasks that ties N threads to the original export call (= "task") that
transitively spawned those N threads. This relationship is used for a number of
...
At any point in time while executing Core WebAssembly code, there is a
well-defined **current thread** whose containing task is the **current task**.
Thus, whenever a [canonical built-in] is called by Core WebAssembly code, it is
meaningful for the built-in to work in terms "the current thread/task".

The "current thread" is modelled in the Canonical ABI's Python code by
explicitly passing a [`Thread`] object as an argument to all function calls so
that the semantic "current thread" is always the value of the `thread`
parameter. Threads store their containing task so that the "current task" is
always `thread.task`.
...
with the thread-local storage array can be eagerly freed by guest code.
Similarly, since each call to an export logically creates a fresh thread,
thread-local allocations can be eagerly released when this implicit thread
exits by returning from the exported function or, if the stackless async ABI is
used, returning the "exit" code to the event loop. Thus, the non-reuse of
thread-local storage between distinct export calls is the key to avoiding what
...
when one of its contained threads is already on the callstack, from
business-as-usual **sibling reentrance**, in which a component instance is
reentered for the first time on a particular async call stack. Recursive
reentrance currently always traps, but will be allowed (and indicated to core
...
* [`thread.switch-to`]: suspend the current thread and immediately resume a
  given thread
* [`thread.suspend`]: suspend the current thread
...
* [`thread.resume-later`]: resume a given thread at some point in the future;
  continue executing in the current thread
* [`thread.yield-to`]: immediately resume a given thread; resume the current
  thread at some point in the future
* [`thread.yield`]: immediately resume *some* (nondeterministically-selected)
  other thread; resume the current thread at some point in the future
...
"main thread" of a task. When the last thread in a task returns, there is a
...
of the subtask's threads and then allowing the subtask to continue executing an
arbitrary amount of time (calling imports, performing I/O and everything else)
until the subtask decides to call [`task.cancel`] to confirm the cancellation
or, for whatever reason, call `task.return` as-if there had been no
cancellation. `task.cancel` enforces the same "all borrowed handles dropped"
...
sentinel value and the caller must [wait](#waiting) until the subtask reaches a
resolved state.
...
The stackful ABI is currently gated by the 🧵 (threading) feature.
