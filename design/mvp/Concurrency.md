...
behalf of by calling the [`thread.set-task`] built-in.
...
the [`thread.set-task`] built-in mentioned above, which may *only* be called by
threads explicitly created by `thread.new-indirect`.
...
`task.return`; there is no "main thread" of a task. When a task contains no
more threads (due to its threads exiting or leaving via `thread.set-task`),
there is a trap if `task.return` (or `task.cancel`, described below) has not
been called by a thread contained by the task.
