...
explainer] for a high-level description of the concurrency model being
specified here.
...
native concurrency support in the Component Model. The expectation is that
the host will interleave calls to `invoke` with calls to `tick`, repeatedly
calling `tick` until there is no more work to do or the store is destroyed.
The nondeterministic `random.shuffle` indicates that the embedder is allowed
to use any algorithm (involving priorities, fairness, etc) to choose which
thread to schedule next (and hopefully an algorithm more efficient than the
simple polling loop written above). The `Thread.ready` and `Thread.resume`
...
TODO: reword
TODO: maybe explain `assert(thread in self.threads and thread.task is self)`
...
[future](Concurrency.md#TODO), there will be a way for a function to opt in
...
TODO: only called by `thread.*` built-ins, so no "until" ready func
```python
...
When a `Task`'s final `Thread` returns, the `trap_if`
...
* 🔀,not(🧵) `async` - `callback` must also be present
...
Once the backpressure gate is cleared, TODO (`thread.index`)
... the arguments are lowered into core wasm
...
concurrent `thread.yield`, `waitable-set.wait`, `waitable-set.poll` or `async
...
def canon_thread_index(thread):
  ...
  return [thread.index]
```
TODO: note useful for getting index of export-created thread (since not
created by `thread.new_indirect`
...
def canon_thread_new_indirect(ft, ftbl: Table[CoreFuncRef], thread, fi, closure):
  assert(ft == CoreFuncType(['i32'], []))
  trap_if(not thread.task.inst.may_leave)
  f = ftbl.get(fi)
  trap_if(f.t != ft)
  def thread_func(thread):
    [] = call_and_trap_on_throw(f.callee, thread, [closure])
...
def canon_thread_resume_later(thread, i):
  ...
  return []
```
TODO: there is no `Task.resume_later` b/c no 'cancellable'
...
For a canonical definition:
```wat
(canon thread.yield $cancellable? (core func $f))
```
validation specifies:
* `$f` is given type `(func (result i32))`
* 🧵 - `cancellable` is allowed (otherwise it must be absent)
...
TODO: change to return the thread index or 0 if fail
```python
...
For a canonical definition:
```wat
(canon thread.available-parallelism (core func $f))
```
validation specifies:
