...
The `Task.needs_lock` predicate returns whether the Canonical ABI options
...
synchronously or with `async callback`. Functions lifted with non-`callback`
`async` ("stackful async") never acquire the lock and thus can aribtrarily
reenter at most import calls (as defined by the `canon_*` definitions
below). This predicate is used by the other `Task` methods to determine whether
to acquire/release a component-instance-wide [`asyncio.Lock`] stored in
...
backpressure is disabled. There are three sources of backpressure:
 ...
 3. *Implicit backpressure* triggered when `Task.needs_lock()` is true and
    the `ComponentInstance.lock` is already held.
...
below is careful to satisfy three requirements:
 1. The component-instance-wide `num_pending` counter tracks how many residual
    tasks are still waiting to start due to explicit backpressure.
...
unblocking. (`unblock` is set by the `async callback` event loop to allow
...
progressing in the same component instance.
```python
