...
`WritableBufferGuestImpl` classes:
```python
...
```
Based on this, constructing a readable or writable buffer from guest code
*eagerly* checks alignment and range so that `read` and `write` are infallible
...
state) until OOM. If `may_enter` (defined below) signals that backpressure is
needed, `enter` blocks by putting itself into the per-`ComponentInstance`
`pending_tasks` queue and blocking until released by `maybe_start_pending_task`
(defined below). The `or self.inst.pending_tasks` disjunct ensures fairness,
preventing continual new tasks from starving pending tasks.
...
```python
  async def wait_on(self, sync, a: Awaitable):
    if self.inst.interruptible.is_set():
      if sync:
        self.inst.interruptible.clear()
        v = await self.on_block(a)
    ...
    return v
```
The `Task.maybe_start_pending_task` method called by `wait_on` above (and
`exit`, below) works in concert with `enter` to start pending tasks. One key
property ensured by `enter`+`may_start`+`maybe_start_pending_task` is that
`pending_tasks` are only allowed to start one at a time, ensuring that if an
overloaded component instance enables backpressures, builds up a large queue of
pending tasks, and then disables backpressure, there will not be a thundering
herd of tasks started all at once that OOM the component before it can
re-enable backpressure. To specify this property, the `starting_pending_task`
flag is set here and cleared when the pending task actually resumes execution
in `enter`, preventing more pending tasks from being started in the interim.
```python
...
for any external I/O (as emulated in this Python code by waiting on
`asyncio.sleep(0)`):
```python
...
```python
  if opts.sync:
    ...
    if e.copy(buffer, on_partial_copy, on_copy_done) != 'done':
      ...
      await task.wait_on(opts.sync, async_copy)
      final_revoke_buffer()
```
...
async def cancel_copy(EndT, event_code, t, sync, task, i):
  ...
  if not e.has_pending_event():
    ...
    if not e.has_pending_event():
      if sync:
        await task.wait_on(sync, e.wait_for_pending_event())
      else:
        return [BLOCKED]
