...
class ReadableFuture:
  t: ValType
  read: Callable[[ComponentInstance, Lower, OnCopyDone], Optional[CopyResult]]
  cancel: Callable[[], None]
  drop: Callable[[]]
```
...
asymmetric difference is that, as mentioend above, only the writable end can
...
```python
  ...
  def write(self, inst, lift, on_copy_done):
    ...
    if self.dropped:
      ...
      return CopyResult.COMPLETED
```
Lastly, the `{Readable,Writable}FutureEnd` classes are almost entirely
symmetric with `{Readable,Writable}StreamEnd`, with the only difference being
that `WritableFutureEnd.drop` traps if the writer hasn't successfully written a
...
class ReadableFutureEnd(FutureEnd):
  def copy(self, inst, lower, on_copy_done):
    return self.shared.read(inst, lower, on_copy_done)
...
class WritableFutureEnd(FutureEnd):
  def copy(self, inst, lift, on_copy_done):
    return self.shared.write(inst, lift, on_copy_done)

  def drop(self):
    trap_if(not self.done)
    FutureEnd.drop(self)
```
The `copying` and `done` fields are mutated by the `future` built-ins defined
below.
...
readable end from the lifting component to the host or lowering component.
```python
...
def lift_async_value(ReadableEndT, cx, i, t):
  ...
  return e.shared
```
There is a trap if the readable end is in the middle of `copying` (which would
otherwise leave a dangling pointer) or the readable end is already `done`
(which is set when the end receives notification that the other end was
`DROPPED` or when a `future` receives its value).
...
```
Next, the `{Readable,Writable}StreamEnd.copy` method is called to attempt to
perform the actual `read` or `write`. The `on_copy` callback passed to `copy`
is called zero or more times each time values are copied to/from `buffer`. The
`on_copy_done` callback passed to `copy` is called at most once when the
read/write is definitely finished.
```python
...
```
If the stream has been dropped or at least 1 element could be immediately
copied, `copy` returns a `CopyResult` which is packed with the number of
elements copied and returned to the caller of `stream.{read,write}`. If the
`CopyResult` was `DROPPED`, then the end is marked `done`, in which case the
only valid operation going forward is `stream.drop-{readable,writable}`.

Otherwise, `copy` blocked:
```python
  else:
    e.copying = True
    if opts.sync:
...
necessarily calls a `stream_event` closure created by either `on_copy` or
...
Regardless of whether the `stream.{read,write}` completes synchronously or
asynchronously, the results passed to core wasm are bit-packed into a single
`i32` according to the following scheme:
```python
...
async def future_copy(EndT, event_code, copy, future_t, opts, task, i):
  ...
  result = e.copy(task.inst, copy, on_copy_done)
  if result is not None:
    assert(result != CopyResult.DROPPED or event_code == EventCode.FUTURE_WRITE)
