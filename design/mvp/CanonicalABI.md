...
There are two sources of backpressure and so `enter` will be introduced in two
corresponding chunks. The first source of backpressure is implicitly triggered
whenever a component is currently in the middle of executing synchronous code
that does not expect to be reentered (e.g., because the code is using a single
global linear memory shadow stack). The Canonical ABI assumes that both
`sync`-lifted and `async callback`-lifted core functions are non-reentrant and
so when either of these `canonopt`s are set, `enter()` `acquire()`s a
component-instance-wide [`asyncio.Lock`] to serialize execution. `async`
non-`callback` ("stackful async") functions do not acquire the lock and thus
can aribtrarily reenter even while synchronous code is executing (with the
burden being on the stackful async code to do the right thing).
```python
  async def enter(self):
    if self.opts.sync or self.opts.callback:
      if self.inst.lock.locked():
        acquired = asyncio.create_task(self.inst.lock.acquire())
        if await self.block_on(acquired, cancellable = True) == Cancelled.TRUE:
          if acquired.done():
...
```
In order to allow other tasks to run in the interim, `enter()` can't simply
`await self.inst.lock.acquire()` but must instead spawn a new [`asyncio.Task`]
that can be passed to `Task.block_on()` (defined next). If the caller cancels
the task while it is blocked in `enter()`, the task is immediately aborted
(which `enter()` achieves by returning `Cancelled.TRUE` to `canon_lift`). In
this case, as a non-semantic implementation detail, the Python code above must
`cancel()` the `acquire()`, taking care to handle the case where `acquire()`
has racily already succeeded. The final `else` branch, when `lock` is
uncontended, `await`s the `acquire()` *without* calling `Task.block_on()`,
which has the semantic effect of *not* officially "blocking" (in the sense
that control flow is immediately returned to any transitive `async` caller).

The second source of backpressure checked by `enter()` is explicitly enabled by
core wasm code calling `backpressure.set`. When passed a non-zero value,
`canon_backpressure_set` (defined below) will clear a per-component-instance
`no_backpressure` [`asyncio.Event`] which `enter()` here `wait()`s to be set
again. The `unblock` parameter is set to tell `block_on` to drop the (just
acquired) `lock` for synchronous code so that active `async callback` tasks can
execute in the meantime. Once `no_backpressure` is `set()`, all blocked tasks
are unblocked. However, due to cooperativity, only one task will be executed at
a time. If, between backpressure being disabled and a previously-blocked task
executing, another task executes and *re*enables backpressure (e.g., due to OOM
conditions being reached again), the previously-blocked task *must* go back to
being blocked. This is achieved in the Python code below by wrapping the
`block_on()` call in a loop that re-checks `no_backpressure`.
```python
...
```
To prevent new tasks from starving pending tasks, `enter()` maintains a
`num_backpressure_waiters` counter and blocks new tasks if this counter is
greater than zero. Tasks are resumed non-deterministically by `block_on()`, so
runtimes are always free to schedule resumption based on prioritization
heuristics; `num_backpressure_waiters` just ensures that the runtime is
*semantically allowed* do so once backpressure has been disabled.
...
runtime to non-deterministically avoid officially "blocking" the task (which,
in component-to-component async calls, means that control flow does necessarily
not need to transfer back to the calling component yet).
...
If `block_on` is called with `unblock` set, then the `ComponentInstance.lock`
...
unblocking. `unblock` is set by `Task.enter()` and the `async callback` event
loop to allow concurrent tasks to execute when the current task blocks at a
reentrancy-safe point.
...
the `ComponentInstance.lock` that was `acquire()`d by `Task.enter()` to
serialize non-reentrant core wasm execution.
```python
  def exit(self):
    trap_if(self.state != Task.State.RESOLVED)
...
implicit or explicit backpressure, as described above in `Task.enter`. If the
supertask cancels the call while waiting for backpressure, the call is
immediately aborted without ever executing the callee's core wasm function.
Only after the backpressure gates have been cleared are the arguments lowered
into core wasm values and memory (according to the `canonopt` immediates of
`canon lift` as defined by `lower_flat_values` above).
...
`ComponentInstance.lock` and the `Task.exit` call here releases the `lock`,
...
that, since `Task.enter` does *not* acquire the `ComponentInstance.lock` for
stackful async functions, calls to `waitable-set.wait` made by a stackful async
function do not prevent other stackful async calls from starting or
progressing.
```python
...
`unlock` flag is only set when calling these blocking operations from the event
loop, not the built-ins. `unblock` tells these blocking operations to release
the `ComponentInstance.lock` taken by `Task.enter()` so that other `async
callback` functions can start or make progress in the interim. Since the
built-ins don't set `unblock`, they do not drop the `lock`, thereby ensuring
the invariant that `async callback` code is never reentered while other `async
callback` core wasm code (or, because they share the same `lock`, synchronous
core wasm code) is on the stack.
...
Note that, since `unblock` is not set, when called from a synchronously-lifted
or `async callback`-lifted export, `yield` will not allow other synchronously-
lifted or `async callback`-lifted code to execute before returning.
...
Note that, since `unblock` is not set, when called from a synchronously-lifted
or `async callback`-lifted export, `waitable-set.wait` will not allow other
synchronously-lifted or `async callback`-lifted code to execute before
returning.
...
Note that, since `unblock` is not set, when called from a synchronously-lifted
or `async callback`-lifted export, `waitable-set.poll` will not allow other
synchronously-lifted or `async callback`-lifted code to execute before
returning.
